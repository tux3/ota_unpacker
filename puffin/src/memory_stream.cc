...
UniqueStreamPtr MemoryViewStream::CreateForRead(const uint8_t* data, size_t length) {
  return UniqueStreamPtr(new MemoryViewStream(data, length));
}

MemoryViewStream::MemoryViewStream(const uint8_t* data, size_t length)
    : data_(data),
      length_(length),
      offset_(0),
      open_(true) {}

bool MemoryViewStream::GetSize(uint64_t* size) const {
  *size = length_;
  return true;
}

bool MemoryViewStream::GetOffset(uint64_t* offset) const {
  *offset = offset_;
  return true;
}

bool MemoryViewStream::Seek(uint64_t offset) {
  TEST_AND_RETURN_FALSE(open_);
  uint64_t size;
  GetSize(&size);
  TEST_AND_RETURN_FALSE(offset <= size);
...
bool MemoryViewStream::Read(void* buffer, size_t length) {
  TEST_AND_RETURN_FALSE(open_);
  TEST_AND_RETURN_FALSE(offset_ + length <= length_);
  memcpy(buffer, data_ + offset_, length);
  offset_ += length;
  return true;
}

bool MemoryViewStream::Write(const void* buffer, size_t length) {
  return false;
}

bool MemoryViewStream::Close() {
  open_ = false;
  return true;
}
