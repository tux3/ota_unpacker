...
class MemoryViewStream : public StreamInterface {
 public:
  ~MemoryViewStream() override = default;

  // Creates a stream for reading.
  static UniqueStreamPtr CreateForRead(const uint8_t* data, size_t length);
 ...
 private:
  // Ctor. Exactly one of the |read_memory| or |write_memory| should be nullptr.
  MemoryViewStream(const uint8_t* data, size_t length);
  ...
  DISALLOW_COPY_AND_ASSIGN(MemoryViewStream);
};
