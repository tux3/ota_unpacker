...
int puffpatch_c(uint8_t const* src, size_t src_len,
                uint8_t const* patch, size_t patch_len,
                const void* userarg,
                uint64_t(*sink)(const void* userarg, const uint8_t*, uint64_t))
{
  auto src_stream = MemoryViewStream::CreateForRead(src, src_len);
  auto dst_stream = SinkStream::Create(userarg, sink);
  return puffin::PuffPatch(std::move(src_stream), std::move(dst_stream), patch, patch_len);
}
